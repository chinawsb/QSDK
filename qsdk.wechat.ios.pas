...
type
  ...
  public
    constructor Create(AResp: BaseResp);
  end;
  ...
  public
    constructor Create(AResp: PayResp);
  ...
  private
    ...
    procedure onReq(P1: BaseReq);
    procedure onResp(P1: BaseResp);
  protected
    { WXApiDelegate }
...
begin
  ...
  begin
    ...
    TWXApi.OCClass.handleOpenURL
      (TNSURL.Wrap(TNSURL.OCClass.URLWithString(StrToNSStr(AContext.url))),
...
procedure TiOSWechatService.onReq(P1: BaseReq);
begin
  if Assigned(FOnRequest) then
    FOnRequest(TiOSWechatRequest.Create(P1));
end;

procedure TiOSWechatService.onResp(P1: BaseResp);
var
  APayResp: PayResp;
begin
  if Assigned(FOnResponse) then
  begin
    if Supports(P1, PayResp, APayResp) then
      FOnResponse(TiWechatPayResponse.Create(APayResp))
    else
      FOnResponse(TiOSWechatResponse.Create(P1));
  end;
end;
...
constructor TiOSWechatResponse.Create(AResp: BaseResp);
begin
  inherited Create;
  FType := AResp._type;
  FErrorCode := AResp.errCode;
  FErrorMsg := NSStrToStr(AResp.errStr);
end;
...
begin
  FService.onResp(resp);
end;

{ TiWechatPayResponse }

constructor TiWechatPayResponse.Create(AResp: PayResp);
var
  APayResp: PayResp;
begin
  inherited Create(AResp);
  FReturnKey := NSStrToStr(AResp.returnKey);
end;
